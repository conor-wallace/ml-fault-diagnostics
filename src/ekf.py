...
class EKF(object):
    ...
    def step(self, z):
        ...
        G = np.dot(self.P_pre * H.T, scipy.linalg.inv(H * self.P_pre * H.T + self.R))
...
class PositionEKF(EKF):
    def __init__(self):
        EKF.__init__(self, 3, 3, pval=0.1, qval=1e-2, rval=0.005)

    def f(self, x):
        return np.copy(x), np.eye(3)
