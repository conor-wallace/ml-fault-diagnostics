...
class Drift_Optimizer():
    def __init__(self):
        self.x_data = []

    def dynamics(self, x, y, theta, v, gamma, dt):
        x = np.clip(x, -1e5, 1e5)
    ...
    def driveOpenLoop(self, gamma, max_iter, logging=False):
        self.x_data = []
        y_data = []
        i = 0
        ...
        while(i != max_iter):
            if logging:
                print("GAMMA CHECK: ", gamma)

            self.x_data.append(x)
            y_data.append(y)
            x, y, theta = self.dynamics(x, y, theta, 0.39, gamma, 0.02)
            i += 1
    ...
    def func(self, x, gamma, logging=False):
        y = self.driveOpenLoop(gamma, x.shape[0], logging)
